...
# Updates in this file vs your previous version:
#   1) Added _rerun() helper for Streamlit compatibility
#   2) Replaced all st.experimental_rerun() -> _rerun()
#   3) Kept earlier fixes: use r["name"] (not r.name) and toggle edit panels
...
def _rerun():
    """Use st.rerun() if available, else fall back to experimental."""
    try:
        st.rerun()
...
def _employee_row_actions(df):
    ...
    for _, r in df.iterrows():
        ...
        col[1].write(r["name"])  # use dict access (fix)
...
# ---------- Main ----------
def main():
    st.set_page_config(page_title="Payroll App (Drive + PDF)", layout="wide")
    ensure_state_keys()
